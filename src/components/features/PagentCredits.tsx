...
export function PagentCredits() {
  ...
  return (
    <div className="space-y-6">
      ...
      >
        <CreditCardMini
          availableCredit={isConnected ? availableCredit : 0}
          totalLimit={isConnected ? creditLimit : 0}
            ...
            <div className="flex gap-2">
              {!isConnected ? (
                <div className="text-center w-full">
                  <p className="text-white/70 text-xs">Connect wallet to manage credits</p>
                </div>
              ) : !hasActivePermission ? (
                <Button
            ...
            <CardContent className="p-6">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <h3 className="font-semibold text-green-800 dark:text-green-200 mb-1">Card Eligible!</h3>
                    <p className="text-sm text-green-700 dark:text-green-300">
                      Your ${formatCurrency(creditLimit)} spend permission qualifies you for Pagent Card
                    </p>
                  </div>
                </div>
                <Button
        ...
        {hasActivePermission && (
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <Card className="p-4 text-center">
              <div className="text-2xl font-bold text-[#6B53FF]">{formatCurrency(creditLimit)}</div>
            ...
            {billingRecords.length > 0 ? (
              <div className="space-y-3">
                {billingRecords.slice(0, 3).map((record) => (
                  <div key={record.id} className="flex items-center justify-between p-3 hover:bg-gray-50 dark:hover:bg-gray-800 rounded-lg transition-colors">
      ...
      {showCreateModal && (
        <CreatePermissionModal
          ...
          onSuccess={(data) => {
            // Refresh data will happen via useEffect when activePermission changes
