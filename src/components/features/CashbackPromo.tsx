...
export function CashbackPromo() {
 ...
 useEffect(() => {
  const loadRewardsData = async () => {
   ...
   try {
    ...
    const result = await SecureAPI.getRewards(session.access_token)
    
    if (result.success) {
     setRewardsData(result.data)
     console.log('✅ Loaded rewards data:', result.data)
    } else {
     console.error('❌ Failed to load rewards data:', result.error)
    }
   } catch (error) {
 ...
 const pgtPoints = rewardsData?.points || 12450
 const pgtValue = pgtPoints * 0.01 // 1 PGT = $0.01
 const totalEarned = rewardsData?.balance ? Math.round(rewardsData.balance * 100) : 5632
 const thisMonthEarned = 245 // This could be calculated from recent cashback
 ...
 // Use real cashback data or fallback to mock data
 const recentCashbacks = rewardsData?.cashback?.map((cb: any) => ({
  id: cb.id,
  source: cb.merchant || 'Unknown Merchant',
  amount: cb.amount,
  pgt: Math.round(cb.amount * 100), // Convert to points
  date: new Date(cb.earned_at).toLocaleDateString()
 })) || [
 ...
 return (
  <div className="space-y-6">
   ...
   <Tabs defaultValue="top" className="w-full">
            <TabsList className="mb-3 bg-gradient-to-r from-gray-50 to-gray-100 p-1">
    ...
    <TabsContent value="top">
     <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
      {topBundles.map((item) => {
       ...
       return (
                        <Card key={item.id} className="group hover:shadow-2xl hover:scale-[1.02] transition-all duration-500 overflow-hidden border border-gray-200/50 min-h-[32rem] flex flex-col relative bg-gradient-to-br from-white to-gray-50">
    ...
    <TabsContent value="picked">
     <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
      {whopPicked.map((item) => {
       const lines = getCashbackLines(item.extraCashback)
       return (
                        <Card key={item.id} className="group hover:shadow-xl hover:scale-[1.01] transition-all duration-300 overflow-hidden border border-gray-200/50 min-h-[18rem] flex flex-col relative bg-gradient-to-br from-white to-gray-50">
         {/* Subtle glow effect */}
          ...
          <div className="flex items-center justify-between">
           ...
           <span className="font-semibold text-sm">{formatCurrency(item.price)}</span>
                              ...
                              <div className="bg-orange-50 rounded-lg p-2">
                                 <ul className="text-[11px] space-y-1 text-orange-700">
   ...
   {/* Active Promos from API */}
   {rewardsData?.promos && rewardsData.promos.length > 0 && (
    <Card>
     <CardHeader>
      <CardTitle className="flex items-center gap-2">
      ...
      <p className="text-sm text-muted-foreground">
       Special offers and cashback bonuses available now
      </p>
     </CardHeader>
     <CardContent>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
       {rewardsData.promos.slice(0, 4).map((promo: any) => (
        <div key={promo.id} className="p-4 border border-gray-200 rounded-lg hover:shadow-md transition-shadow">
         <div className="flex items-start justify-between mb-2">
          ...
          <span className="text-xs bg-green-100 text-green-800 px-2 py-1 rounded-full">
           {promo.reward_type === 'cashback' ? `${promo.reward_value}% back` : 
            promo.reward_type === 'points' ? `${promo.reward_value} pts` : 
            `${promo.reward_value}x bonus`}
          </span>
         </div>
         <p className="text-xs text-muted-foreground mb-2">{promo.description}</p>
         <p className="text-xs text-gray-600">{promo.conditions}</p>
    ...
    <CardContent>
     <div className="space-y-3">
      {recentCashbacks.map((cashback) => (
                     <div key={cashback.id} className="flex items-center justify-between p-3 hover:bg-gray-50 rounded-lg transition-colors">
    ...
    <CardContent className="p-6">
     <div className="flex items-center justify-between mb-4">
      <div className="flex items-center gap-4">
       ...
       <div>
                        <h3 className="font-semibold text-purple-800 mb-1">Gold Status Benefits</h3>
